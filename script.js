...
const product = {
  ...
  getSkuFromProductItem: (item) => item.querySelector('span.item__sku').innerText,
};
...
const cart = {
  ...
  createElement: ({ sku, name, salePrice }) => {
    const li = document.createElement('li');
    li.className = 'cart__item';
    li.innerText = `SKU: ${sku} | NAME: ${name} | PRICE: $${salePrice}`;
    li.addEventListener('click', cart.itemClickListener);
    return li;
  },
  add: (e) => {
    ...
    product.get(product.getSkuFromProductItem(e.target.closest('.item')))
  ...
  addElement: (dados) => carrinhoElem.appendChild(cart.createElement(dados)),
  calc: () => {
    const ls = JSON.parse(localStorage.getItem('carrinho') || '[]');
    const total = ls.reduce((sum, obj) => obj.salePrice + sum, 0);
    document.querySelector('.total-price')
...
product.createElement = ({ sku, name, image }) => {
...
window.onload = () => {
  ...
  product.list().then((response) => response.json()).then((produtos) => {
    ...
    p.forEach((el) => {
      ...
      .appendChild(
        product.createElement({ sku: el.id, name: el.title, image: el.thumbnail }),
      );
    });
  }).catch((error) => console.log('error', error));
